...
from starzel.deformwidgets.dynatree import MultiSelectDynatreeWidget, \
...
# XXX Wrong place wrong way to get vocab
...
class DeformHelpers(grok.View):
    ...
    def deform_form(self):
           ...
           # fields = fieldset_mapping.get(fieldset.__name__, [])
            ...
            if fields:
                ...
                if not fieldset.__name__ in [x[1] for x in fieldsets]:
                    fieldsets.append((order_sum / hits, fieldset.__name__))
        fieldsets.sort()
        ...
        return translatedForm(schema, ('submit', 'cancel', ), self.context)
...
class BaseEditForm(DeformHelpers):
    ...
    def handleSubmit(self):
        ...
        try:
            appstruct = flatten_cstruct(self.deform_form.validate(form_data))
